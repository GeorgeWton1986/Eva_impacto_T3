...
# Función para calcular errores estándar clusterizados
...
mod_3_3 <- lm(bizexpense_1 ~ treatment + area_pop_base + area_business_total_base + 
...
mod_3_4 <- lm(bizprofit_1 ~ treatment + area_pop_base + area_business_total_base + 
                 area_exp_pc_mean_base + area_literate_head_base + 
                 area_literate_base, 
                 data = el1_data, 
                weights = el1_data$w1)

#Errores clustereados
results_expenses <- cluster_se(mod_3_3, el1_data$areaid)
results_profit <- cluster_se(mod_3_4, el1_data$areaid)

round(results_expenses[2,], 4)
round(results_profit[2,], 4)

#Extraer información de manera organizada
coef_expenses <- round(coef(mod_3_3)["treatment"], 4)
se_expenses <- round(sqrt(diag(vcovCL(mod_3_3, cluster = el1_data$areaid)))["treatment"], 4)
pval_expenses <- round(results_expenses[2,4], 4)

coef_profit <- round(coef(mod_3_4)["treatment"], 4)
se_profit <- round(sqrt(diag(vcovCL(mod_3_4, cluster = el1_data$areaid)))["treatment"], 4)
pval_profit <- round(results_profit[2,4], 4)
